...
const Add_PurchasesProducts = ({ToggleAlert}) => {
  ...
  const GenerateOrdernumber = (e) => {
    e.length !== 0 ? e.map((userx, index) => (
      e.length - 1 === index ? setinvoice_final(userx.product + 1) : ""
  ...
  const handleChange = (e) => {
    ...
    setshowformdata({ ...showformdata, [e.target.name]: e.target.value, user_id: user._id, currency: "Pakistani Rupees", net: net, amount: amount, product: invoice_final });
  }
...
export default Add_PurchasesProducts;

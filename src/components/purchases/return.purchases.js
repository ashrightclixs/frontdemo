...
const Purchases_Return = ({ ToggleAlert }) => {
  ...
  const show = (value,command) => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/purchase/return/getrefundpage?page=${value}&id=${user_id}`, {
      'user_id': user_id
    }).then((response) => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/purchase/return?id=${user_id}`, {
      'user_id': user_id
    }).then((response) => {
  ...
  const approved_delete = (value) => {
    axios.delete(`${process.env.REACT_APP_API_URL}/purchase/invoice/deleterefundarray?id=${value}`, {
      'user_id': user_id
    }).then((response) => {
      show(1,"delete");
    })
  ...
  const delete_data = async (id) => {
    ...
    try {
      const url = `${process.env.REACT_APP_API_URL}/purchase/return/delete?id=${id}`;
...
export default Purchases_Return;
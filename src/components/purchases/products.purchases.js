...
const Purchases_Products = ({ ToggleAlert }) => {
  ...
  const filterdata = (val) => {
    if (customer && search != "") {
      ...
      return val.product == qoutation_value;
    } else {
      return val;
    }
  }
  ...
  const paginate = (value) => {
    setpagination(value);
    show(value,"NotDelete");
  }

  const show = (value,command) => {

    if(command == "delete"){
      alert("Are You sure you want to delete data");
    }
    axios.get(`${process.env.REACT_APP_API_URL}/products/getproductpage?page=${value}&id=${user_id}`, {
      'user_id': user_id
    }).then((response) => {
  ...
  const approved_delete = (value) => {
    axios.delete(`${process.env.REACT_APP_API_URL}/products/deleteproductarray?id=${value}`, {
      'user_id': user_id
    }).then((response) => {
      show(1,"delete");
    })
  ...
  const delete_data = async (id) => {
    ...
    try {
      const url = `${process.env.REACT_APP_API_URL}/products/delete?id=${id}`;
        ...
        <div className="px-md-4 pt-md-4 px-1 pt-1">
          <div className="d-flex justify-content-between flex-md-row flex-column my-2 border-bottom py-3">
            ...
            <div className="d-flex justify-content-end flex-md-row flex-column">
              <Link to="/purchases/add_products">
          ...
          <div className={filter == true ? "d-flex justify-content-between flex-md-row flex-column my-2 py-3" : "d-none"}>
            <div className="text-center my-2 my-md-0 order-last order-md-first">
              ...
              <button type="button"
                ...
                <i class={customer === true ? "fa fa-check" : "fa fa-times"} aria-hidden="true"></i>    WareHouse
              </button>
            </div>
          </div>
...
export default Purchases_Products;;
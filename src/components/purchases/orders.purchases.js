...
const Purchases_Orders = ({ ToggleAlert }) => {
  ...
  const show = (value) => {
    axios.get(`${process.env.REACT_APP_API_URL}/purchase/order/getorderpage?page=${value}&id=${user_id}`, {
      'user_id': user_id
    }).then((response) => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/purchase/order?id=${user_id}`, {
      'user_id': user_id
    }).then((response) => {
  ...
  const approved_delete = (value) => {
    axios.delete(`${process.env.REACT_APP_API_URL}/purchase/order/deleteorderarray?id=${value}`, {
      'user_id': user_id
    }).then((response) => {
      show(1);
    })
  ...
  const delete_data = async (id) => {
    try {
      const url = `${process.env.REACT_APP_API_URL}/purchase/order/delete?id=${id}`;
          ...
          <div className={filter == true ? "d-flex justify-content-between flex-md-row flex-column my-2 py-3" : "d-none"}>
            <div className="text-center my-2 my-md-0 order-last order-md-first">
              ...
              <button type="button"
                ...
                <i class={customer === true ? "fa fa-check" : "fa fa-times"} aria-hidden="true"></i>    Products
              </button>
            </div>
          </div>
...
export default Purchases_Orders;;
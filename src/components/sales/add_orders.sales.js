...
const Add_SalesOrders = ({ ToggleAlert }) => {
  ...
  useEffect(() => {
    ...
    // setentriesdata
    axios
      .get(`${process.env.REACT_APP_API_URL}/sales/entries`, {
        user_id: user_id,
      })
      .then((response) => {
    ...
    //productshow  product
    axios
      .get(`${process.env.REACT_APP_API_URL}/products?id=${user_id}`, {
        user_id: user_id,
      })
      .then((response) => {
        ...
        // if(response.data.length === 0){
        //   setTimeout(() => {
        //     ToggleAlert("Warning","please add products first")
        //   }, 10000);
      })
      .catch((e) => {
        console.log("error");
  ...
  const handleChange = (e) => {
    ...
    setentries({
      ...entries,
      user_id: user._id,
      account_receivable: received_value,
      date: getdate(),
      account_payable: net - received_value,
      amount: net - (net - received_value),
      order: order_final,
    });
    setshowformdata({
      ...showformdata,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = `${process.env.REACT_APP_API_URL}/sales/order/add`;
      const response = await axios.post(url, showformdata);
      setshowformdata(initialState);

      try {
        const url_entries = `${process.env.REACT_APP_API_URL}/sales/entries/add`;
        const responseentries = await axios.post(url_entries, entries);
      ...
      {loader_val === false && (
        <>
          <div className="main-card">
            {/* <div
          className={
            customer.length === 0 || tax.length === 0 || product.length === 0
...
export default Add_SalesOrders;

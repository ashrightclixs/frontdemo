...
const SalesInvoice = ({ ToggleAlert }) => {
  ...
  const show = (value) => {
    axios.get(`${process.env.REACT_APP_API_URL}/sales/invoice/getinvoicepage?page=${value}&id=${user_id}`, {
      'user_id': user_id
    }).then((response) => {
  ...
  const handleSubmitRecived = async (e) => {
    ...
    if (net_amount < parseInt(recived_amount) + recived) {
      ...
      ToggleAlert(
        "Warning",
        "Amount received must not be greater than amount due"
      );
    } else {
      try {
        const url = `${process.env.REACT_APP_API_URL}/sales/invoice/check_invoice?id=${invoice_id}&recived_money=${recived_amount}`;
  ...
  const approved_delete = (value) => {
    axios.delete(`${process.env.REACT_APP_API_URL}/sales/invoice/deleteinvoicearray?id=${value}`, {
      'user_id': user_id
    }).then((response) => {
      setIsCheck([]);
      show(1);
  ...
  const delete_data = async (id) => {
    ...
    try {
      const url = `${process.env.REACT_APP_API_URL}/sales/invoice/delete?id=${id}`;
          ...
          <button type="button" className="close" data-dismiss="alert" aria-label="Close">
                        ...
                        <div className="col-md-12 my-4">
                          ...
                          <div className="input-group">
                            <input
                              ...
                              name="due_date"
                              className="form-control"
                              placeholder="Enter amount received recently"
            ...
            <div className="px-md-4 pt-md-4 px-2 pt-2">
              <div className="d-flex justify-content-between flex-row my-2 border-bottom py-3">
                <h4 className="mb-md-0 text-center text-md-left">Sales Invoice</h4>
                <Link to="/sales/add_invoice" className="text-center">
                  <button

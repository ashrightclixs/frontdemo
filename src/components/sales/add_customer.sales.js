...
const Add_SalesCustomer = ({ToggleAlert}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const url = `${process.env.REACT_APP_API_URL}/customer/add`;
        const response = await axios.post(url , data);
        setData(initialState);
        setLoader(false)
        navigate('/sales/customer');
        setTimeout(() => {
          ToggleAlert("Success", "Customer added successfully");
        }, 1000);
    } 
    catch (e) {
      console.log(error)
                ...
                <tr>
                  <td>
                    <input
                      ...
                      className="form-control"
                      id="validationCustomUsername"
                      placeholder="Enter customer's company/shop name"
                      name="company"
                      onChange={handleChange}
                      aria-describedby="inputGroupAppend"
                      style={styles.uiInput}
...
export default Add_SalesCustomer;